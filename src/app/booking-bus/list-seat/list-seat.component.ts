...
export class ListSeatComponent implements OnInit {
  @ViewChildren(SeatComponent) tagListSeat: QueryList<SeatComponent> =
    new QueryList();
  ...
  listSeat: any = [
    ...
    { SoGhe: 8, TenGhe: 'số 8', Gia: 100, TrangThai: true },
  ...
  soGheDaDat: number = 0;
  tongTien: number = 0;
  DanhSachGheDangDat = Array();
  constructor() {}

  ngOnInit(): void {}

  datGheParent(status: any, seat: any) {
    if (status) {
      this.soGheDaDat++;
      this.DanhSachGheDangDat.push(seat);
      this.tongTien += seat.Gia;
    } else {
      this.soGheDaDat--;
      this.tongTien -= seat.Gia;

      for (let index in this.DanhSachGheDangDat) {
        if (this.DanhSachGheDangDat[index].SoGhe === seat.SoGhe) {
  ...
  huyGhe(ghe: any) {
    ...
    this.tagListSeat.forEach((item: any) => {
      if (item.seat.SoGhe === ghe.SoGhe) {
        item.status = false;
      }
    });
  }
